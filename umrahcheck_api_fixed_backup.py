...
async def scrape_hotel_prices_endpoint(request: HotelPriceRequest):
    """
    Get hotel prices using multiple sources:
    1. RapidAPI (primary) - reliable and legal
    2. Playwright scraping (fallback) - when API fails
    """
    try:
        logger.info(f"🏨 Price request for {request.hotel_name}")
        ...
        if use_rapidapi and rapidapi_key:
            logger.info("🚀 Using RapidAPI for price data")
            
            try:
                from rapidapi_booking_apidojo import BookingApiDojo
                
                api = BookingApiDojo(rapidapi_key)
                api_result = await api.search_hotels_by_name(
                    hotel_name=request.hotel_name,
                    city=request.city,
                    checkin=request.checkin_date,
                    checkout=request.checkout_date,
                ...
                if api_result.get('status') == 'success':
                    ...
                    best_price = api_result.get('best_price', {})
                    ...
                    return {
                        ...
                        "data": {
                            ...
                            "checkin": request.checkin_date,
                            "checkout": request.checkout_date,
                            "nights": api_result.get('nights', 1),
                            "adults": request.adults,
                            "rooms": request.rooms,
                            "children": request.children,
                            "currency": request.currency,
                            "results": [
                                {
                                    ...
                                    "price_per_night": result.get('price_per_night', 0),
                                    "total_price": result.get('total_price', 0),
                                    "currency": result.get('currency', 'EUR'),
                                    "availability": result.get('available', True),
                                    "url": result.get('url', ''),
                            ...
                            "best_price": {
                                "platform": "Booking.com (RapidAPI)",
                                "price_per_night": best_price.get('price_per_night', 0),
                                "total_price": best_price.get('total_price', 0),
                                "currency": best_price.get('currency', 'EUR'),
                                "url": best_price.get('url', ''),
                                "review_score": best_price.get('review_score', 0),
                                "nights": api_result.get('nights', 1),
                                "exact_match": best_price.get('exact_match', False)
                    ...
                    logger.warning(f"RapidAPI failed: {api_result.get('message')}")
                    
            except Exception as api_error:
                logger.warning(f"RapidAPI error: {api_error}")
        ...
        with sentry_sdk.start_span(op="api.scrape", description=f"Scrape {request.hotel_name}"):
            ...
            return {
                "status": "success",
                "data": scraped_data,
                "message": "Prices scraped via Playwright (fallback mode)"
            }
...
async def get_statistics():
    ...
    rapidapi_available = bool(os.getenv("RAPIDAPI_KEY")) and os.getenv("USE_RAPIDAPI", "true").lower() == "true"
    playwright_available = os.getenv("ENABLE_PLAYWRIGHT", "false").lower() == "true"
    
    return {
        "api_status": "operational",
        "airtable_connected": True,
        "rapidapi_available": rapidapi_available,
        "playwright_available": playwright_available,
        "features": {
            "live_pricing": "rapidapi_primary" if rapidapi_available else ("real" if playwright_available else "simulated"),
            "recommendations": "active_fixed",
            "commission_tracking": "ready",
            "field_mapping": "flexible",
            "web_scraping": "fallback" if rapidapi_available else ("enabled" if playwright_available else "disabled"),
            "api_integration": "booking.com_apidojo" if rapidapi_available else "none"
        },
        "version": "1.3.0 - RapidAPI Integration",
        "timestamp": datetime.now().isoformat()
    }
